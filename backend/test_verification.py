...
def run_tests():
    ...
    resp_a = requests.post(f"{BASE_URL}/documents/7/verify")
    ...
    pre_verify = requests.post(f"{BASE_URL}/documents/{tamper_doc_id}/verify").json()
    ...
    resp_b = requests.post(f"{BASE_URL}/documents/{tamper_doc_id}/verify")
    ...
    # Clean up fixture file after test
    if os.path.exists(fixture_path):
        os.remove(fixture_path)

    # --- TEST CASE C: NONEXISTENT DOCUMENT ID ---
    print("\n[TEST CASE C] Nonexistent Document ID (999999)...")
    resp_c = requests.post(f"{BASE_URL}/documents/999999/verify")
    ...
    resp_d = requests.post(f"{BASE_URL}/documents/{tamper_doc_id}/verify")
